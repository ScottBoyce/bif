...
MODULE GENERIC_INPUT_FILE_INSTRUCTION!, ONLY: GENERIC_INPUT_FILE                        --side note, reposition STREAM with READ(IU,'()', ADVANCE='NO', POS=P)
  ...
  USE FILE_IO_INTERFACE,      ONLY: READ_TO_DATA, MAX_LINE_LENGTH, COMMENT_INDEX, GET_FILE_NAME
  USE PARSE_WORD_INTERFACE,   ONLY: PARSE_WORD
  USE STRINGS,                ONLY: UPPER, SPECIAL_BLANK_STRIP, GET_NUMBER, GET_INTEGER
  USE ERROR_INTERFACE,        ONLY: FILE_IO_ERROR, GET_WARN
  ...
  CONTAINS
  !
  RECURSIVE SUBROUTINE OPEN_GENERIC_INPUT_FILE(FL, LN, LLOC, OUTPUT, INFILE, NOSTOP, REQKEY, NOSFAC, IU, BINARY, BUFFER, EOL, NO_INTERNAL, NO_ONLY_UNIT, NO_CONSTANT, NEW_UNIT, SAVE_FNAME, KEY, DIM, PREPOST, STREAM_TEXT, NOPOSTKEY, FORCE_DATAFILE, MSG)
    ...
    !
    LOGICAL:: ISOPEN, ALLOW_ERROR, NOREQKEY, FOUND_KEY, DATAFILE, GO_TO_TOP, EXIST, REQ_NEW_UNIT, CHECK_POST, CHECK_ONLY_UNIT, NOT_UNIQUE, SAVE_FN
