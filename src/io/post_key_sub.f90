...
MODULE POST_KEY_SUB!, ONLY: CHECK_FOR_POST_KEY, FILE_AND_POST_KEY_PARSE
  ...
  CONTAINS
  !
  SUBROUTINE CHECK_FOR_POST_KEY(LLOC, LN, IN, IOUT, BUF, SPLIT, SCALE, FILSTAT, FILACT, ASYN, BINARY, NOPRINT, GO_TO_TOP, DIM, OLDLOC, FMT, ONLY_CHECK, NO_WARN, MSG)
    ...
    DOUBLE PRECISION,          INTENT(INOUT), OPTIONAL:: SCALE     ! IS RETURNED IF A SCALE FACTOR IS FOUND, OTHERWISE LEFT ALONE. --Set to 1.0 on entry!!!
    ...
    LOGICAL,                   INTENT(  OUT), OPTIONAL:: GO_TO_TOP               ! GO_TO_TOP IS SET TO TRUE IF KEYWORD FOUND, OTHERWISE LEFT ALONE
    ...
    LOGICAL,                   INTENT(IN   ), OPTIONAL:: NO_WARN
    CHARACTER(*),              INTENT(IN   ), OPTIONAL:: MSG
    CHARACTER(32):: KEY  !Note 32 is hardwired into case check
    CHARACTER(:), ALLOCATABLE:: ERR
    INTEGER:: I, N, ISTART, ISTOP, LLOC_BAK, IERR, C
    DOUBLE PRECISION:: SF
    LOGICAL:: SKIP, ALLOW_WARNING
    !
    SKIP = FALSE
    IF(PRESENT(ONLY_CHECK)) SKIP = ONLY_CHECK
    !
    ALLOW_WARNING = TRUE
    IF(PRESENT(NO_WARN)) SKIP = .not. NO_WARN
    !
    !Z = 0
    ...
    LLOC = LLOC_BAK
    !
    IF(ERR.NE.NL .and. ALLOW_WARNING) CALL WARNING_MESSAGE(LN,IN,IOUT,MSG='...CHECK_FOR_POST_KEY WARNINGS...'//NL//' THE FOLLOWING MESSAGES WERE PASSED WHILE CHECKING FOR ANY ADDITIONAL KEYWORDS ON LOADED LINE.'//BLN//ERR//BLN//'--Note that the check for post keywords continues until either the end of line is reached or when comment symbol, #, is found.'//NL//'If you want to keep the added text to the right, but disable this warning, you should precede it with a #')
    !
  END SUBROUTINE
  !
