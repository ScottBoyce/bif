...
MODULE GENERIC_OUTPUT_FILE_INSTRUCTION!, ONLY: GENERIC_OUTPUT_FILE
  ...
  END SUBROUTINE
  !
  SUBROUTINE OPEN_GENERIC_OUTPUT_FILE(FL, LN, LLOC, OUTPUT, INFILE, NOSTOP, REQKEY, IU, BINARY, BUFFER, SPLIT_SIZE, NOBINARY, SPLITMAXCOUNT, NO_INTERNAL, NO_ONLY_UNIT, SAVE_FNAME, KEY, DIM, PREPOST, STREAM_TEXT, NOPOSTKEY, FORCE_DATAFILE, MSG)
    ...
    LOGICAL:: ISOPEN, ALLOW_ERROR, NOREQKEY, DATAFILE, FOUND_KEY, CHECK_POST, CHECK_ONLY_UNIT, NOT_UNIQUE, SAVE_FN
    CHARACTER(12):: FORM_CHK, EXT, ACCESS_TXT
    ...
    IF(.NOT. ISOPEN) THEN
            ...
            IF (IERR .EQ. Z) THEN
                !
                FL%IU = IU_READ 
                IF(CHECK_POST) CALL CHECK_FOR_POST_KEY(LL,LN,IIN,FL%IOUT,BUF,SPLIT,DIM=DIM,FMT=FL%FMT,OLDLOC=PREPOST, MSG=MSG)
                !
                IF (FL%IU == Z) THEN
            ...
            ELSE
                ...
                ELSEIF(EXT == 'EXTERNAL' .OR. EXT=='DATAUNIT') THEN
                                                 ...
                                                 IF(.NOT. FL%ERROR .AND. CHECK_POST) CALL CHECK_FOR_POST_KEY(LL,LN,IIN,FL%IOUT,BUF,SPLIT,DIM=DIM,FMT=FL%FMT,OLDLOC=PREPOST, MSG=MSG)
                                                 FOUND_KEY=TRUE
                ELSE
                      ...
                      !
                      IF(CHECK_POST) CALL CHECK_FOR_POST_KEY(LL,LN,IIN,FL%IOUT,BUF,SPLIT,BINARY=FL%BINARY,DIM=DIM,FMT=FL%FMT,OLDLOC=PREPOST, MSG=MSG)  !FL%BINARY IS ONLY SET TO TRUE IF BINARY FLAG FOUND, OTHERWISE IGNORED
                      !
