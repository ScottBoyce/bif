...
MODULE NUM2STR_INTERFACE!, ONLY: NUM2STR, NUM2STR7, INTFMT, NUMFMT
  ...
  PURE FUNCTION REL32_2STR(RVAL,PAD,GENERAL,LS,RS) RESULT(STR)
    ...
    ELSE
    !
    RTMP =   10._rel32*RVAL;  RVAL10_CHK = ABS(RTMP - ANINT(RTMP)) < ABS(RTMP) * 1.e-7_rel32 .and. ( RTMP>=1._rel32 .or. RTMP<=-1._rel32 )
    RTMP =  100._rel32*RVAL;  RVAL1C_CHK = ABS(RTMP - ANINT(RTMP)) < ABS(RTMP) * 1.e-7_rel32 .and. ( RTMP>=1._rel32 .or. RTMP<=-1._rel32 )
    RTMP = 1000._rel32*RVAL;  RVAL1K_CHK = ABS(RTMP - ANINT(RTMP)) < ABS(RTMP) * 1.e-7_rel32 .and. ( RTMP>=1._rel32 .or. RTMP<=-1._rel32 )
    !
    IF    ( RVAL10_CHK ) THEN
  ...
  PURE FUNCTION REL64_2STR(DVAL,PAD,GENERAL,LS,RS) RESULT(STR)
    ...
    ELSE
    !
    DTMP =     10._rel64*DVAL;    DVAL10_CHK = ABS(DTMP - ANINT(DTMP)) < ABS(DTMP) * 1.e-13_rel64 .and. ( DTMP>=UNO .or. DTMP<=DNEG )
    DTMP =    100._rel64*DVAL;    DVAL1C_CHK = ABS(DTMP - ANINT(DTMP)) < ABS(DTMP) * 1.e-13_rel64 .and. ( DTMP>=UNO .or. DTMP<=DNEG )
    DTMP =   1000._rel64*DVAL;    DVAL1K_CHK = ABS(DTMP - ANINT(DTMP)) < ABS(DTMP) * 1.e-13_rel64 .and. ( DTMP>=UNO .or. DTMP<=DNEG )
    DTMP = 100000._rel64*DVAL;  DVAL100K_CHK = ABS(DTMP - ANINT(DTMP)) < ABS(DTMP) * 1.e-13_rel64 .and. ( DTMP>=UNO .or. DTMP<=DNEG )
    !
    IF( DVAL10_CHK ) THEN
