...
!
!       Submodule Defines SORT routines for REAL(REAL32) Type --> Often this also is the same as the REAL type too
!
!-----------------------------------------------------------------------------------------------------------
!
...
!
!    Rename SUBMODULE (SORT_INTERFACE) SORT_INTERFACE_REL32 
!        to MODULE SORT_INTERFACE_REL32 
!      and
!           END SUBMODULE
!       to  END    MODULE
! 
...
SUBMODULE (SORT_INTERFACE) SORT_INTERFACE_REL32
  ...
  !
  REAL(REL32), PARAMETER:: ZER =  0.0_rel32
  REAL(REL32), PARAMETER:: NEG = -1.0_rel32
  !
  ...
  !\!!
  !\!INTERFACE SORT
  !\!  MODULE PROCEDURE SORT_1D_REL32            ! SORT(A, [DESCEND],     [STABLE],                [P])
  !\!  MODULE PROCEDURE SORT_2D_INDEX_REL32      ! SORT(A, SORT_INDEX,   [DESCEND], [SORT_BY_ROW], [P])
  !\!  MODULE PROCEDURE SORT_2D_MULTI_REL32      ! SORT(A, SORT_INDICES, [DESCEND], [SORT_BY_ROW], [P])  -> DESCEND is array of SIZE(SORT_INDICES)
  !\!  MODULE PROCEDURE SORT_2D_MULTI_REL32_1DSC ! SORT(A, SORT_INDICES, [DESCEND], [SORT_BY_ROW], [P])  -> DESCEND is a scalar
  !\!  MODULE PROCEDURE SORT_1D_QSORT_A_REL32    ! SORT(dim, A)
  !\!  MODULE PROCEDURE SORT_1D_SymMg_P_REL32    ! SORT(dim, A, P)
  !\!END INTERFACE
  !\!!
  !\!INTERFACE SORTED
  !\!  MODULE PROCEDURE SORTED_1D_REL32          ! SORTED(A, DESCEND) RESULT(SORTED_ARRAY)
  !\!END INTERFACE
  !\!!
  !\!INTERFACE REVERSE_ORDER
  !\!  MODULE PROCEDURE REVERSE_ORDER_1D_NODIM_REL32        ! REVERSE_ORDER(A, [STABLE], [P])
  !\!  MODULE PROCEDURE REVERSE_ORDER_1D_DIM_REL32          ! REVERSE_ORDER(dim, A, [STABLE], [P])
  !\!  MODULE PROCEDURE REVERSE_ORDER_2D_REL32              ! REVERSE_ORDER(A, REVERSE_EACH_ROW)  --> Default REVERSE_EACH_ROW is TRUE
  !\!  MODULE PROCEDURE REVERSE_ORDER_2D_INDEX_REL32        ! REVERSE_ORDER(A, REV_INDEX, STABLE, REVERSE_ROW, P)
  !\!END INTERFACE
  !\!!
  !\!INTERFACE PERMUTATION_SORT
  !\!  MODULE PROCEDURE PERMUTATION_SORT_1D_REL32  ! INDEX_SORT(A, P)
  !\!  MODULE PROCEDURE PERMUTATION_SORT_2D_REL32  ! INDEX_SORT(A, P, [BY_COLUMN])   Default is BY_COLUMN=TRUE
  !\!END INTERFACE
  !\!!
  !\!INTERFACE ALLOCATE_P
  !\!  MODULE PROCEDURE ALLOCATE_PERM_REL32 ! ALLOCATE_P(A, IDX)
  !\!END INTERFACE
  !
  CONTAINS  !================================================================================================================
  !
  ...
  MODULE PURE SUBROUTINE SORT_1D_REL32(A, DESCEND, STABLE, P, SORT_TYPE, QSORT, HEAP, ISORT, SymMerge, INIT_P)
    ...
    if(present(SORT_TYPE)) then
         sTYP = SORT_TYPE
    ELSE
         sTYP = SORT_TYPE_REL32(dim, A, DESCEND, STABLE, QSORT, HEAP, ISORT, SymMerge)
    END IF
    !   0 - Array is sorted
    ...
    IF    (sTYP == 1) THEN
                     IF( HAS_P ) THEN
                         CALL ISORT_1P_REL32(dim, A, P)
                     ELSE
                         CALL ISORT_1D_REL32(dim, A)
                     END IF
    ELSEIF(sTYP == 4) THEN
                     IF( HAS_P ) THEN
                         CALL QSORT_1P_REL32( dim, A, P, 1, dim, MAX_RECUR(dim) )
                     ELSE
                         CALL QSORT_1D_REL32( dim, A,      1, dim, MAX_RECUR(dim) )
                     END IF
    ELSEIF(sTYP == 3) THEN
                     IF( HAS_P ) THEN
                         CALL SymMerge_1P_REL32(dim, A, P)
                     ELSE
                         CALL SymMerge_1D_REL32(dim, A)
                     END IF
    ELSEIF(sTYP == 2) THEN
                     IF( HAS_P ) THEN
                         CALL HEAPSORT_1P_REL32(dim, A, P)
                     ELSE
                         CALL HEAPSORT_1D_REL32(dim, A)
                     END IF
    END IF
    !
  ...
  MODULE PURE FUNCTION SORTED_1D_REL32(A, DESCEND, STABLE) RESULT(SORTED_ARRAY)
    ...
    SORTED_ARRAY = A
    CALL SORT_1D_REL32(SORTED_ARRAY, DESCEND=DESCEND, STABLE=STABLE)
  END FUNCTION
  !
  !##########################################################################################################################
  ...
  ! ISORT Routines
  !
  PURE SUBROUTINE ISORT_1D_REL32(dim,A)
    INTEGER,                       intent(in   ) :: dim
    REAL(REL32),    dimension(dim),intent(inout) :: A
    REAL(REL32):: TMP
    INTEGER:: I, J
  ...
  !
  PURE SUBROUTINE ISORT_1P_REL32(dim, A, P)
    INTEGER,                       intent(in   ) :: dim
    REAL(REL32),    dimension(dim),intent(inout) :: A   ! Array to be sorted
  ...
  !
  PURE SUBROUTINE PARTITION_REL32(dim, A, L, R, I, J)
    INTEGER,                        intent(in   ) :: dim, L, R
    REAL(REL32),    dimension(dim), intent(inout) :: A
    INTEGER,                        intent(inout) :: I, J ! Pivot locations
    REAL(REL32):: tmp, Pivot1, Pivot2       ! Pivot Values
    INTEGER:: K
    ...
    !
    IF( A(e2) < A(e1) ) CALL SWAP_REL32( dim, A, e1, e2 ) ! SWAP_REL32( A(e1), A(e2) )
    !
    IF( A(e3) < A(e2) ) THEN
  ...
  !
  PURE SUBROUTINE PARTITION_wP_REL32(dim, A, P, L, R, I, J)
    INTEGER,                        intent(in   ) :: dim, L, R
    REAL(REL32),    dimension(dim), intent(inout) :: A
    INTEGER,        dimension(dim), intent(inout) :: P
    INTEGER,                        intent(inout) :: I, J ! Pivot locations
  ...
  ! QSORT base routines
  !
  RECURSIVE PURE SUBROUTINE QSORT_1D_REL32(dim, A, L, R, D)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    ...
    !
    CALL PARTITION_REL32(dim, A, L, R, I, J)
    !
    CALL QSORT_1D_REL32(dim, A, L  , I-1, D-1)
    CALL QSORT_1D_REL32(dim, A, I+1, J-1, D-1)
    CALL QSORT_1D_REL32(dim, A, J+1,   R, D-1)
    !
  END SUBROUTINE
  !
  !--------------------------------------------------------------------------------------------------------------------------
  ...
  !
  RECURSIVE PURE SUBROUTINE QSORT_1P_REL32(dim, A, P, L, R, D)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    INTEGER,        dimension(dim), intent(inout) :: P
    INTEGER,                        intent(in   ) :: L, R   ! Lower Bound, Upper Bound -> Starting left index and right index
    ...
    IF( K <= QSORT_PARTITION_MIN) THEN   ! => Do Insertion Sort and Return   -- Side note: (R - L + 1 <= 16)
       CALL ISORT_1P_REL32( K, A(L:R), P(L:R) )
       RETURN
    END IF
    !
    IF( D < 1 ) THEN   ! No more recursion allowed, switch to Heap Sort
       CALL HEAPSORT_1P_REL32( K, A(L:R), P(L:R) )
       RETURN
    END IF
    !
    ...
    !
    CALL PARTITION_wP_REL32(dim, A, P, L, R, I, J)
    !
    CALL QSORT_1P_REL32(dim, A, P, L  , I-1, D-1)
    CALL QSORT_1P_REL32(dim, A, P, I+1, J-1, D-1)
    CALL QSORT_1P_REL32(dim, A, P, J+1,   R, D-1)
    !
  END SUBROUTINE
  !
  !##########################################################################################################################
  ...
  !
  PURE SUBROUTINE HEAPSORT_1D_REL32(dim, A)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    REAL(REL32):: TMP
    INTEGER:: I, P, C
  ...
  !
  PURE SUBROUTINE HEAPSORT_1P_REL32(dim, A, PRM)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
  ...
  !
  PURE SUBROUTINE SymMerge_1D_REL32(dim, A)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    ...
    DO I = 1, N, nblock
                 CALL ISORT_1D_REL32(nblock, A(I:I+nb))
    END DO
    nb = dim-N
    IF( nb > 1 )  CALL ISORT_1D_REL32(nb, A(N+1:dim))
    !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    DO WHILE (nblock < dim)
        !
        ...
        !
        DO I = 1, N, nblock2
                     CALL SymMerge_REL32(dim, A, I, I+nblock, I+nb)
        END DO
        N  = N + 1
        nb = N+nblock
        IF( nb <= dim )  CALL SymMerge_REL32(dim, A, N, nb, dim)
        !
        nblock = nblock2
    END DO
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  !
  RECURSIVE PURE SUBROUTINE SymMerge_REL32(dim, A, fi, si, li)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    ...
    if (li - fi == 1) then                                                                         ! Check 3 -> Only two values and from to Check 2, A(fi) > A(si) = A(li)
                      CALL SWAP_REL32( dim, A, fi, li )  ! Check 2 failure implies: A(fi) > A(li)  !            SWAP_REL32( A(fi), A(li) )
                      RETURN                                                                       !
    end if                                                                                         !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
    ...
    if (si == li) then                                                                             ! Check 5 -> Right block only contains one value. Check indicates it needs to be placed in left block.
        ...
        DO m=si-1, n, -1                                                                           !
                    CALL SWAP_REL32(dim, A, m, m+1)                                                !  SWAP_REL32(A(m), A(m+1))
        END DO                                                                                     !
        RETURN                                                                                     !
    end if                                                                                         !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
    ...
    !
    CALL SymMerge_REL32(dim, A, fi, strt, m)
    !
    m = m + 1;  term = term + 1
    !
    CALL SymMerge_REL32(dim, A, m, term, li)
    !
  END SUBROUTINE
  !
  !##########################################################################################################################
  ...
  !
  PURE SUBROUTINE SymMerge_1P_REL32(dim, A, P)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    INTEGER,        dimension(dim), intent(inout) :: P
    INTEGER:: I, nblock, nblock2, nb, N
    ...
    !
    DO I = 1, N, nblock
                 CALL ISORT_1P_REL32(nblock, A(I:I+nb), P(I:I+nb))
    END DO
    nb = dim-N
    IF( nb > 1 )  CALL ISORT_1P_REL32(nb, A(N+1:dim), P(N+1:dim))
    !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    DO WHILE (nblock < dim)
        !
        ...
        !
        DO I = 1, N, nblock2
                     CALL SymMergeP_REL32(dim, A, P, I, I+nblock, I+nb)
        END DO
        N  = N + 1
        nb = N+nblock
        IF( nb <= dim )  CALL SymMergeP_REL32(dim, A, P, N, nb, dim)
        !
        nblock = nblock2
    END DO
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  !
  RECURSIVE PURE SUBROUTINE SymMergeP_REL32(dim, A, P, fi, si, li)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A        ! Array to be sorted
    INTEGER,        dimension(dim), intent(inout) :: P        ! Index Position Array
    INTEGER,                        intent(in   ) :: fi, si, li
    ...
    !
    CALL SymMergeP_REL32(dim, A, P, fi, strt, m)
    !
    m = m + 1;  term = term + 1
    !
    CALL SymMergeP_REL32(dim, A, P, m, term, li)
    !
  END SUBROUTINE
  !
  !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  MODULE PURE SUBROUTINE SORT_2D_INDEX_REL32(A, SORT_INDEX, DESCEND, SORT_BY_ROW, P, NO_STABLE)
    ...
    IF( dim1 == 1 .OR. dim2 == 1 ) THEN  ! Array really is 1D, so use those optimized routines
        IF(dim1 > 1) THEN
                CALL SORT_1D_REL32( A(:,1), DESCEND, STABLE=STABLE, P=P)
        ELSE
                CALL SORT_1D_REL32( A(1,:), DESCEND, STABLE=STABLE, P=P)
        END IF
        RETURN
    ...
    !
    IF( SORT_BY_COL ) THEN
        CALL SORT_2D_COL_REL32(dim1, dim2, A, sortIDX, STABLE, DESCEND, P)
    ELSE
        CALL SORT_2D_ROW_REL32(dim1, dim2, A, sortIDX, STABLE, DESCEND, P)
    END IF
    !
  END SUBROUTINE
  !
  ...
  !
  PURE SUBROUTINE SORT_2D_COL_REL32(dim1, dim2, A, COL, STABLE, DESCEND, PRM)
    INTEGER,                                        intent(in   ) :: dim1, dim2, COL
    REAL(REL32),    dimension(dim1,dim2),           intent(inout) :: A
    ...
    INTEGER:: k
    !
    CALL SORT_1D_REL32(A(:,COL), DESCEND, STABLE=STABLE, P=P)
    !
    IF(present(PRM)) THEN
  ...
  !
  PURE SUBROUTINE SORT_2D_ROW_REL32(dim1, dim2, A, ROW, STABLE, DESCEND, PRM)
    ...
    REAL(REL32),    dimension(max(dim1,dim2)):: TMP
    INTEGER:: i, j, k
    !
    ...
    !END DO
    !
    CALL SORT_1D_REL32(TMP(1:dim2), DESCEND, STABLE=STABLE, P=P)
    !
    IF(present(PRM)) THEN
  ...
  MODULE PURE SUBROUTINE SORT_2D_MULTI_REL32_1DSC(A, SORT_INDICES, DESCEND, SORT_BY_ROW, P, NO_STABLE)
    ...
    CALL SORT_2D_MULTI_REL32(A, SORT_INDICES, DESCEND_ORDER, SORT_BY_ROW, P, NO_STABLE)
  END SUBROUTINE
  !
  MODULE PURE SUBROUTINE SORT_2D_MULTI_REL32(A, SORT_INDICES, DESCEND, SORT_BY_ROW, P, NO_STABLE)    ! SORT_INDICES must contain unique values
    ...
    IF( dim1 == 1 .OR. dim2 == 1 ) THEN  ! Array really is 1D, so use those optimized routines
        ...
        IF(dim1 > 1) THEN                   !DESCEND=
                CALL SORT_1D_REL32( A(:,1), SORT_BY_COL, STABLE=STABLE, P=P)
        ELSE
                CALL SORT_1D_REL32( A(1,:), SORT_BY_COL, STABLE=STABLE, P=P)
        END IF
        RETURN
    ...
    !
    IF( sdim == 1 ) THEN
        CALL SORT_2D_INDEX_REL32(A, SORT_INDICES(1), DCEND(1), SORT_BY_ROW, P, NO_STABLE)
        RETURN
    END IF
    !
    ...
    !
    IF( SORT_BY_COL ) THEN
             CALL SORT_2D_COLDIM_REL32(dim1,dim2,A,sdim,SORT_DIM,DCEND,STABLE,P)
    ELSE
             CALL SORT_2D_ROWDIM_REL32(dim1,dim2,A,sdim,SORT_DIM,DCEND,STABLE,P)
    END IF
    !
  END SUBROUTINE
  !
  ...
  !
  PURE SUBROUTINE SORT_2D_COLDIM_REL32(dim1, dim2, A, sdim, COLDIM, DESCEND, STABLE, PRM)
    ...
    !
    CALL SORT_1D_REL32( A(:,COLDIM(1)), DESCEND(1), STABLE=STABLE, P=P)
    !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       ...
       DIM_SEARCH: DO WHILE (I <  dim1)  ! A(I:J, col) contains the range of repeated values
          ...
          ELSE                                        ! Repeated values from I to J
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1P_REL32   (N, A(I:J, col), P(I:J))
              ELSE
                  CALL SymMerge_1P_REL32(N, A(I:J, col), P(I:J))
              END IF
          END IF
          !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          IF( DESCEND(S) ) A(I:J, col) = -A(I:J, col)
  ...
  !
  PURE SUBROUTINE SORT_2D_ROWDIM_REL32(dim1, dim2, A, sdim, ROWDIM, DESCEND, STABLE, PRM)
    INTEGER,                                   intent(in   ) :: dim1, dim2, sdim
    REAL(REL32),    dimension(dim1,dim2),      intent(inout) :: A
    ...
    call ROW_COPY(dim1, dim2, row, A, TMP)
    !
    call SORT_1D_REL32( TMP, DESCEND(1), STABLE=STABLE, P=P)
    !
    ...
    DO rdim=2, sdim        ! Sort the remaining columns
       ...
       DIM_SEARCH: DO WHILE (I <  dim2)  ! A(row, I:J) contains the range of repeated values
          ...
          ELSE                                                    ! Repeated values from I to J
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1P_REL32   (N, TMP(I:J), P(I:J))
              ELSE
                  CALL SymMerge_1P_REL32(N, TMP(I:J), P(I:J))
              END IF
          END IF
          !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          IF( DESCEND(S) ) TMP(I:J) = -TMP(I:J)
  ...
  MODULE PURE SUBROUTINE REVERSE_ORDER_1D_NODIM_REL32(A, STABLE, P, INIT_P)
    ...
    !
    IF(HAS_P) THEN
              CALL REVERSE_ORDER_1D_DIM_REL32(dim, A, STABLE, P, INIT_P)
    ELSE
              CALL REVERSE_ORDER_1D_DIM_REL32(dim, A, STABLE)
    END IF
    !
  END SUBROUTINE
  !
  MODULE PURE SUBROUTINE REVERSE_ORDER_1D_DIM_REL32(dim, A, STABLE, P, INIT_P)
    ...
    END IF
    !
    CALL REVERSE_ORDER_1D_REL32(dim, A)
    !
    ...
    IF(present(STABLE)) THEN
      IF(STABLE) THEN       ! Search for repeated values and reverse their order so they appear in the original placement order
           ...
           ISEARCH: DO WHILE (I < dim)      ! A(I:J) contains the range of repeated values
              ...
              !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
              N = J - I + 1
              CALL REVERSE_ORDER_1D_REL32( N, A(I:J) )
              !
  ...
  END SUBROUTINE
  !
  PURE SUBROUTINE REVERSE_ORDER_1D_REL32(dim, A)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A
    REAL(REL32):: TMP
  ...
  MODULE PURE SUBROUTINE REVERSE_ORDER_2D_REL32(A, REVERSE_EACH_ROW)
    ...
    IF(present(REVERSE_EACH_ROW)) THEN
      IF(REVERSE_EACH_ROW)  THEN
                            CALL REVERSE_ORDER_2D_EACH_ROW_REL32(dim1,dim2,A)
                            RETURN
      END IF
    END IF
    !
    CALL REVERSE_ORDER_2D_EACH_COL_REL32(dim1,dim2,A)
    !
  END SUBROUTINE
  !
  !--------------------------------------------------------------------------------------------------------------------------
  ...
  MODULE PURE SUBROUTINE REVERSE_ORDER_2D_INDEX_REL32(A, REV_INDEX, STABLE, REVERSE_ROW, P)
    ...
    IF( dim1 == 1 .OR. dim2 == 1 ) THEN  ! Array really is 1D, so use those optimized routines
        IF(dim1 > 1) THEN
                CALL REVERSE_ORDER_1D_NODIM_REL32( A(:,1), STABLE, P )
        ELSE
                CALL REVERSE_ORDER_1D_NODIM_REL32( A(1,:), STABLE, P )
        END IF
        RETURN
    ...
    !
    IF( revCOL ) THEN
        CALL REVERSE_ORDER_2D_COL_REL32(dim1, dim2, A, revIDX, STABLE, P)
    ELSE
        CALL REVERSE_ORDER_2D_ROW_REL32(dim1, dim2, A, revIDX, STABLE, P)
    END IF
    !
  END SUBROUTINE
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_COL_REL32(dim1, dim2, A, COL, STABLE, PRM)
    INTEGER,                                            intent(in   ) :: dim1, dim2, COL
    REAL(REL32),    dimension(dim1,dim2),               intent(inout) :: A
    LOGICAL,                                  optional, intent(in   ) :: STABLE
    ...
    INTEGER:: k
    !
    CALL REVERSE_ORDER_1D_DIM_REL32(dim1, A(:,COL), STABLE, P)
    !
    ! Sort the other columns by permutation vector
    DO k=1, COL-1
                CALL PERM_SORT(dim1, A(:,k), P)
  ...
  !--------------------------------------------------------------------------------------------------------------------------
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_ROW_REL32(dim1, dim2, A, ROW, STABLE, PRM)
    INTEGER,                                            intent(in   ) :: dim1, dim2, ROW
    REAL(REL32),    dimension(dim1,dim2),               intent(inout) :: A
    LOGICAL,                                  optional, intent(in   ) :: STABLE
    INTEGER,        dimension(:), contiguous, optional, intent(inout) :: PRM
    ...
    call ROW_COPY(dim1, dim2, row, A, TMP)
    !
    call REVERSE_ORDER_1D_DIM_REL32(dim2, TMP(1:dim2), STABLE, P)
    !
    IF(present(PRM))THEN
  ...
  !--------------------------------------------------------------------------------------------------------------------------
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_EACH_COL_REL32 (dim1,dim2,A)
  ...
  !--------------------------------------------------------------------------------------------------------------------------
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_EACH_ROW_REL32(dim1, dim2, A)
    INTEGER,                              intent(in   ) :: dim1, dim2
    REAL(REL32),    dimension(dim1,dim2), intent(inout) :: A
    INTEGER:: I, J, HALF_DIM
    !
    ...
    !
    DO I=1, HALF_DIM
            CALL VECTOR_SWAP_REL32( dim1, A(:,I), A(:,J) )
            J = J - 1
    END DO
    !
  END SUBROUTINE
  ...
  PURE SUBROUTINE RotrateBlock(dim, A, L, M, R)
    ...
    IF( M == R ) THEN
                 DO I=R-1, L, -1
                         CALL SWAP_REL32(dim, A, I, I+1)     ! CALL SWAP_REL32(A(I), A(I+1))
                 END DO
                 RETURN
    END IF
    !
  ...
  PURE SUBROUTINE BlockSwap(dim, A, fi, si, siz)
    ...
    INTEGER:: I
    DO I=0, siz-1
            CALL SWAP_REL32(dim, A, fi+I, si+I)     !CALL SWAP_REL32(A(fi+I), A(si+I))
    END DO
  END SUBROUTINE
  !
  !##########################################################################################################################
  ...
  !
  !PURE SUBROUTINE SWAP_REL32(A, B)          ! Can cause compiler errors due to vectorization when passing array parts, such as CALL SWAP_REL32( A(fi+I), A(si+I) )
  !   REAL(REL32),    intent(inout):: A, B
  !   REAL(REL32):: t
  !   t = A
  !   A = B
  !   B = t
  ...
  END SUBROUTINE
  !
  PURE SUBROUTINE VECTOR_SWAP_REL32(dim, A, B)
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(inout) :: A, B
    REAL(REL32):: TMP
    INTEGER:: I
    !
    DO I=1, dim
  ...
  PURE SUBROUTINE ROW_COPY_TO_VEC(dim1, dim2, row, A, B)
    INTEGER,                               intent(in ) :: dim1, dim2, row
    REAL(REL32),    dimension(dim1, dim2), intent(in ) :: A
    REAL(REL32),    dimension(dim2),       intent(out) :: B
    INTEGER:: i
    !
  ...
  PURE SUBROUTINE ROW_COPY_FROM_VEC(dim1, dim2, row, A, B)
    INTEGER,                               intent(in ) :: dim1, dim2, row
    REAL(REL32),    dimension(dim2),       intent(in ) :: A
    REAL(REL32),    dimension(dim1, dim2), intent(out) :: B
    INTEGER:: i
    !
    DO CONCURRENT(i=1:dim2)
                  B(row,i) = A(i)
  ...
  !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  !
  ! SORT_B_by_A_and_P_xyz_REL32 Routines
  !
    ...
    DIM_SEARCH: DO WHILE (I < dim)  ! A(I:J, col) contains the range of repeated values
       ...
       IF(HAS_P) THEN
          ...
          ELSE                                        ! Repeated values from I to J
              ...
              !
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1P_REL32   (N, B(I:J), P(I:J))
              ELSE
                  CALL SymMerge_1P_REL32(N, B(I:J), P(I:J))
              END IF
              !
              IF(dscnd) B(I:J) = -B(I:J)
          END IF
       ELSE!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          ELSE                                        ! Repeated values from I to J
              ...
              !
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1D_REL32   (N, B(I:J))
              ELSE
                  CALL SymMerge_1D_REL32(N, B(I:J))
              END IF
              !
              IF(dscnd) B(I:J) = -B(I:J)
          END IF
  ...
  !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  !
  ! SORT_B_by_REP_and_P_REL32 Routines
  !
  ...
  END SUBROUTINE
  !
  PURE SUBROUTINE UPDATE_REP_REL32(dim, A, REP, repeat_found) !Given a sorted A, update REP to include its repeated values that match with non-zero REP
    INTEGER,                        intent(in   ) :: dim
    REAL(REL32),    dimension(dim), intent(in   ) :: A
    INTEGER,        dimension(dim), intent(inout) :: REP
  ...
  MODULE PURE SUBROUTINE SORT_B_by_A_and_P_REP_REL32(dim, REP, B, repeat_sort, P, descend)
    ...
    END DO DIM_SEARCH
    !
    IF(repeat_found) CALL UPDATE_REP_REL32(dim, B, REP, repeat_found)
    !
  ...
  !!
  !PURE FUNCTION REPEATED_RANGE_REL32(dim, A, I) RESULT(J)  ! aka JSEARCH
