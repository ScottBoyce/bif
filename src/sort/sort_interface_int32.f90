...
!
!       Submodule Defines SORT routines for INTEGER(INT32) Type --> Often this also is the same as the INTEGER type too
!
!-----------------------------------------------------------------------------------------------------------
!
...
!
!    Rename SUBMODULE (SORT_INTERFACE) SORT_INTERFACE_INT32
!        to MODULE SORT_INTERFACE_INT32
!      and
!           END SUBMODULE
!       to  END    MODULE
!
...
!-----------------------------------------------------------------------------------------------------------
!
SUBMODULE (SORT_INTERFACE) SORT_INTERFACE_INT32
  ...
  !\!!
  !\!INTERFACE SORT
  !\!  MODULE PROCEDURE SORT_1D_INT32            ! SORT(A, [DESCEND],     [STABLE],                [P])
  !\!  MODULE PROCEDURE SORT_2D_INDEX_INT32      ! SORT(A, SORT_INDEX,   [DESCEND], [SORT_BY_ROW], [P])
  !\!  MODULE PROCEDURE SORT_2D_MULTI_INT32      ! SORT(A, SORT_INDICES, [DESCEND], [SORT_BY_ROW], [P])  -> DESCEND is array of SIZE(SORT_INDICES)
  !\!  MODULE PROCEDURE SORT_2D_MULTI_INT32_1DSC ! SORT(A, SORT_INDICES, [DESCEND], [SORT_BY_ROW], [P])  -> DESCEND is a scalar
  ...
  !\!  MODULE PROCEDURE REVERSE_ORDER_1D_NODIM_INT32        ! REVERSE_ORDER(A, [STABLE], [P])
  !\!  MODULE PROCEDURE REVERSE_ORDER_1D_DIM_INT32          ! REVERSE_ORDER(dim, A, [STABLE], [P])
  !\!  MODULE PROCEDURE REVERSE_ORDER_2D_INT32              ! REVERSE_ORDER(A, REVERSE_EACH_ROW)  --> Default REVERSE_EACH_ROW is TRUE
  !\!  MODULE PROCEDURE REVERSE_ORDER_2D_INDEX_INT32        ! REVERSE_ORDER(A, REV_INDEX, STABLE, REVERSE_ROW, P)
  !\!END INTERFACE
  !\!!
  !\!INTERFACE PERMUTATION_SORT
  !\!  MODULE PROCEDURE PERMUTATION_SORT_1D_INT32  ! INDEX_SORT(A, P)
  !\!  MODULE PROCEDURE PERMUTATION_SORT_2D_INT32  ! INDEX_SORT(A, P, [BY_COLUMN])   Default is BY_COLUMN=TRUE
  !\!END INTERFACE
  !\!!
  !\!INTERFACE ALLOCATE_P
  !\!  MODULE PROCEDURE ALLOCATE_PERM_INT32 ! ALLOCATE_P(A, IDX)
  !\!END INTERFACE
  !
  CONTAINS  !================================================================================================================
  !
  ...
  MODULE PURE SUBROUTINE SORT_1D_INT32(A, DESCEND, STABLE, P, SORT_TYPE, QSORT, HEAP, ISORT, SymMerge, INIT_P)
    ...
    if(present(SORT_TYPE)) then
         sTYP = SORT_TYPE
    ELSE
         sTYP = SORT_TYPE_INT32(dim, A, DESCEND, STABLE, QSORT, HEAP, ISORT, SymMerge)
    END IF
    !   0 - Array is sorted
    ...
    IF    (sTYP == 1) THEN
                     IF( HAS_P ) THEN
                         CALL ISORT_1P_INT32(dim, A, P)
                     ELSE
                         CALL ISORT_1D_INT32(dim, A)
                     END IF
    ELSEIF(sTYP == 4) THEN
                     IF( HAS_P ) THEN
                         CALL QSORT_1P_INT32( dim, A, P, 1, dim, MAX_RECUR(dim) )
                     ELSE
                         CALL QSORT_1D_INT32( dim, A,      1, dim, MAX_RECUR(dim) )
                     END IF
    ELSEIF(sTYP == 3) THEN
                     IF( HAS_P ) THEN
                         CALL SymMerge_1P_INT32(dim, A, P)
                     ELSE
                         CALL SymMerge_1D_INT32(dim, A)
                     END IF
    ELSEIF(sTYP == 2) THEN
                     IF( HAS_P ) THEN
                         CALL HEAPSORT_1P_INT32(dim, A, P)
                     ELSE
                         CALL HEAPSORT_1D_INT32(dim, A)
                     END IF
    END IF
    !
  ...
  MODULE PURE FUNCTION SORTED_1D_INT32(A, DESCEND, STABLE) RESULT(SORTED_ARRAY)
    ...
    SORTED_ARRAY = A
    CALL SORT_1D_INT32(SORTED_ARRAY, DESCEND=DESCEND, STABLE=STABLE)
  END FUNCTION
  !
  !##########################################################################################################################
  ...
  ! ISORT Routines
  !
  PURE SUBROUTINE ISORT_1D_INT32(dim,A)
    INTEGER,                       intent(in   ) :: dim
    INTEGER(INT32), dimension(dim),intent(inout) :: A
    INTEGER(INT32):: TMP
    INTEGER:: I, J
  ...
  !
  PURE SUBROUTINE ISORT_1P_INT32(dim, A, P)
    INTEGER,                       intent(in   ) :: dim
    INTEGER(INT32), dimension(dim),intent(inout) :: A   ! Array to be sorted
  ...
  !
  PURE SUBROUTINE PARTITION_INT32(dim, A, L, R, I, J)
    INTEGER,                        intent(in   ) :: dim, L, R
    INTEGER(INT32), dimension(dim), intent(inout) :: A
    INTEGER,                        intent(inout) :: I, J ! Pivot locations
    INTEGER(INT32):: tmp, Pivot1, Pivot2       ! Pivot Values
    INTEGER:: K
    ...
    !
    IF( A(e2) < A(e1) ) CALL SWAP_INT32( dim, A, e1, e2 ) ! SWAP_INT32( A(e1), A(e2) )
    !
    IF( A(e3) < A(e2) ) THEN
  ...
  !
  PURE SUBROUTINE PARTITION_wP_INT32(dim, A, P, L, R, I, J)
    INTEGER,                        intent(in   ) :: dim, L, R
    INTEGER(INT32), dimension(dim), intent(inout) :: A
    INTEGER,        dimension(dim), intent(inout) :: P
    INTEGER,                        intent(inout) :: I, J ! Pivot locations
  ...
  RECURSIVE PURE SUBROUTINE QSORT_1D_INT32(dim, A, L, R, D)
    ...
    !
    CALL PARTITION_INT32(dim, A, L, R, I, J)
    !
  ...
  !
  RECURSIVE PURE SUBROUTINE QSORT_1P_INT32(dim, A, P, L, R, D)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A
    INTEGER,        dimension(dim), intent(inout) :: P
    INTEGER,                        intent(in   ) :: L, R   ! Lower Bound, Upper Bound -> Starting left index and right index
    ...
    IF( K <= QSORT_PARTITION_MIN) THEN   ! => Do Insertion Sort and Return   -- Side note: (R - L + 1 <= 16)
       CALL ISORT_1P_INT32( K, A(L:R), P(L:R) )
       RETURN
    END IF
    !
    IF( D < 1 ) THEN   ! No more recursion allowed, switch to Heap Sort
       CALL HEAPSORT_1P_INT32( K, A(L:R), P(L:R) )
       RETURN
    END IF
    !
    ...
    !
    CALL PARTITION_wP_INT32(dim, A, P, L, R, I, J)
    !
    CALL QSORT_1P_INT32(dim, A, P, L  , I-1, D-1)
    CALL QSORT_1P_INT32(dim, A, P, I+1, J-1, D-1)
    CALL QSORT_1P_INT32(dim, A, P, J+1,   R, D-1)
    !
  END SUBROUTINE
  !
  !##########################################################################################################################
  ...
  !
  PURE SUBROUTINE HEAPSORT_1P_INT32(dim, A, PRM)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A
  ...
  !
  PURE SUBROUTINE SymMerge_1D_INT32(dim, A)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A
    ...
    DO I = 1, N, nblock
                 CALL ISORT_1D_INT32(nblock, A(I:I+nb))
    END DO
    nb = dim-N
    IF( nb > 1 )  CALL ISORT_1D_INT32(nb, A(N+1:dim))
    !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    DO WHILE (nblock < dim)
        !
        ...
        !
        DO I = 1, N, nblock2
                     CALL SymMerge_int32(dim, A, I, I+nblock, I+nb)
        END DO
        N  = N + 1
        nb = N+nblock
        IF( nb <= dim )  CALL SymMerge_int32(dim, A, N, nb, dim)
        !
        nblock = nblock2
    END DO
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  !
  RECURSIVE PURE SUBROUTINE SymMerge_int32(dim, A, fi, si, li)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A
    ...
    if (li - fi == 1) then                                                                         ! Check 3 -> Only two values and from to Check 2, A(fi) > A(si) = A(li)
                      CALL SWAP_INT32( dim, A, fi, li )  ! Check 2 failure implies: A(fi) > A(li)  !            SWAP_INT32( A(fi), A(li) )
                      RETURN                                                                       !
    end if                                                                                         !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
    ...
    if (si == li) then                                                                             ! Check 5 -> Right block only contains one value. Check indicates it needs to be placed in left block.
        ...
        DO m=si-1, n, -1                                                                           !
                    CALL SWAP_INT32(dim, A, m, m+1)                                                !  SWAP_INT32(A(m), A(m+1))
        END DO                                                                                     !
        RETURN                                                                                     !
    end if                                                                                         !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
    ...
    !
    CALL SymMerge_int32(dim, A, fi, strt, m)
    !
    m = m + 1;  term = term + 1
    !
    CALL SymMerge_int32(dim, A, m, term, li)
    !
  END SUBROUTINE
  !
  !##########################################################################################################################
  ...
  !
  PURE SUBROUTINE SymMerge_1P_INT32(dim, A, P)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A
    INTEGER,        dimension(dim), intent(inout) :: P
    INTEGER:: I, nblock, nblock2, nb, N
    ...
    !
    DO I = 1, N, nblock
                 CALL ISORT_1P_INT32(nblock, A(I:I+nb), P(I:I+nb))
    END DO
    nb = dim-N
    IF( nb > 1 )  CALL ISORT_1P_INT32(nb, A(N+1:dim), P(N+1:dim))
    !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    DO WHILE (nblock < dim)
        !
        ...
        !
        DO I = 1, N, nblock2
                     CALL SymMergeP_int32(dim, A, P, I, I+nblock, I+nb)
        END DO
        N  = N + 1
        nb = N+nblock
        IF( nb <= dim )  CALL SymMergeP_int32(dim, A, P, N, nb, dim)
        !
        nblock = nblock2
    END DO
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  !
  RECURSIVE PURE SUBROUTINE SymMergeP_int32(dim, A, P, fi, si, li)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A        ! Array to be sorted
    INTEGER,        dimension(dim), intent(inout) :: P        ! Index Position Array
    INTEGER,                        intent(in   ) :: fi, si, li
    ...
    !
    CALL SymMergeP_int32(dim, A, P, fi, strt, m)
    !
    m = m + 1;  term = term + 1
    !
    CALL SymMergeP_int32(dim, A, P, m, term, li)
    !
  END SUBROUTINE
  !
  !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  MODULE PURE SUBROUTINE SORT_2D_INDEX_INT32(A, SORT_INDEX, DESCEND, SORT_BY_ROW, P, NO_STABLE)
    ...
    IF( dim1 == 1 .OR. dim2 == 1 ) THEN  ! Array really is 1D, so use those optimized routines
        IF(dim1 > 1) THEN
                CALL SORT_1D_INT32( A(:,1), DESCEND, STABLE=STABLE, P=P)
        ELSE
                CALL SORT_1D_INT32( A(1,:), DESCEND, STABLE=STABLE, P=P)
        END IF
        RETURN
    ...
    !
    IF( SORT_BY_COL ) THEN
        CALL SORT_2D_COL_INT32(dim1, dim2, A, sortIDX, STABLE, DESCEND, P)
    ELSE
        CALL SORT_2D_ROW_INT32(dim1, dim2, A, sortIDX, STABLE, DESCEND, P)
    END IF
    !
  END SUBROUTINE
  !
  ...
  !
  PURE SUBROUTINE SORT_2D_COL_INT32(dim1, dim2, A, COL, STABLE, DESCEND, PRM)
    INTEGER,                                        intent(in   ) :: dim1, dim2, COL
    INTEGER(INT32), dimension(dim1,dim2),           intent(inout) :: A
    ...
    INTEGER:: k
    !
    CALL SORT_1D_INT32(A(:,COL), DESCEND, STABLE=STABLE, P=P)
    !
    IF(present(PRM)) THEN
  ...
  !
  PURE SUBROUTINE SORT_2D_ROW_INT32(dim1, dim2, A, ROW, STABLE, DESCEND, PRM)
    ...
    INTEGER(INT32), dimension(max(dim1,dim2)):: TMP
    INTEGER:: i, j, k
    !
    ...
    !END DO
    !
    CALL SORT_1D_INT32(TMP(1:dim2), DESCEND, STABLE=STABLE, P=P)
    !
    IF(present(PRM)) THEN
  ...
  MODULE PURE SUBROUTINE SORT_2D_MULTI_INT32_1DSC(A, SORT_INDICES, DESCEND, SORT_BY_ROW, P, NO_STABLE)
    ...
    CALL SORT_2D_MULTI_INT32(A, SORT_INDICES, DESCEND_ORDER, SORT_BY_ROW, P, NO_STABLE)
  END SUBROUTINE
  !
  MODULE PURE SUBROUTINE SORT_2D_MULTI_INT32(A, SORT_INDICES, DESCEND, SORT_BY_ROW, P, NO_STABLE)    ! SORT_INDICES must contain unique values
    ...
    IF( dim1 == 1 .OR. dim2 == 1 ) THEN  ! Array really is 1D, so use those optimized routines
        ...
        IF(dim1 > 1) THEN                   !DESCEND=
                CALL SORT_1D_INT32( A(:,1), SORT_BY_COL, STABLE=STABLE, P=P)
        ELSE
                CALL SORT_1D_INT32( A(1,:), SORT_BY_COL, STABLE=STABLE, P=P)
        END IF
        RETURN
    ...
    !
    IF( sdim == 1 ) THEN
        CALL SORT_2D_INDEX_INT32(A, SORT_INDICES(1), DCEND(1), SORT_BY_ROW, P, NO_STABLE)
        RETURN
    END IF
    !
    ...
    !
    IF( SORT_BY_COL ) THEN
             CALL SORT_2D_COLDIM_INT32(dim1,dim2,A,sdim,SORT_DIM,DCEND,STABLE,P)
    ELSE
             CALL SORT_2D_ROWDIM_INT32(dim1,dim2,A,sdim,SORT_DIM,DCEND,STABLE,P)
    END IF
    !
  END SUBROUTINE
  !
  ...
  !
  PURE SUBROUTINE SORT_2D_COLDIM_INT32(dim1, dim2, A, sdim, COLDIM, DESCEND, STABLE, PRM)
    ...
    !
    CALL SORT_1D_INT32( A(:,COLDIM(1)), DESCEND(1), STABLE=STABLE, P=P)
    !
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       ...
       DIM_SEARCH: DO WHILE (I <  dim1)  ! A(I:J, col) contains the range of repeated values
          ...
          ELSE                                        ! Repeated values from I to J
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1P_INT32   (N, A(I:J, col), P(I:J))
              ELSE
                  CALL SymMerge_1P_INT32(N, A(I:J, col), P(I:J))
              END IF
          END IF
          !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          IF( DESCEND(S) ) A(I:J, col) = -A(I:J, col)
  ...
  !
  PURE SUBROUTINE SORT_2D_ROWDIM_INT32(dim1, dim2, A, sdim, ROWDIM, DESCEND, STABLE, PRM)
    INTEGER,                                   intent(in   ) :: dim1, dim2, sdim
    INTEGER(INT32), dimension(dim1,dim2),      intent(inout) :: A
    ...
    INTEGER,        dimension(:),    optional, intent(inout) :: PRM
    INTEGER,        dimension(dim2):: P
    INTEGER(INT32), dimension(dim2):: TMP
    ...
    call ROW_COPY(dim1, dim2, row, A, TMP)
    !
    call SORT_1D_INT32( TMP, DESCEND(1), STABLE=STABLE, P=P)
    !
    ...
    DO rdim=2, sdim        ! Sort the remaining columns
       ...
       DIM_SEARCH: DO WHILE (I <  dim2)  ! A(row, I:J) contains the range of repeated values
          ...
          ELSE                                                    ! Repeated values from I to J
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1P_INT32   (N, TMP(I:J), P(I:J))
              ELSE
                  CALL SymMerge_1P_INT32(N, TMP(I:J), P(I:J))
              END IF
          END IF
          !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          IF( DESCEND(S) ) TMP(I:J) = -TMP(I:J)
  ...
  MODULE PURE SUBROUTINE REVERSE_ORDER_1D_NODIM_INT32(A, STABLE, P, INIT_P)
    ...
    !
    IF(HAS_P) THEN
              CALL REVERSE_ORDER_1D_DIM_INT32(dim, A, STABLE, P, INIT_P)
    ELSE
              CALL REVERSE_ORDER_1D_DIM_INT32(dim, A, STABLE)
    END IF
    !
  END SUBROUTINE
  !
  MODULE PURE SUBROUTINE REVERSE_ORDER_1D_DIM_INT32(dim, A, STABLE, P, INIT_P)
    ...
    END IF
    !
    CALL REVERSE_ORDER_1D_INT32(dim, A)
    !
    ...
    IF(present(STABLE)) THEN
      IF(STABLE) THEN       ! Search for repeated values and reverse their order so they appear in the original placement order
           ...
           ISEARCH: DO WHILE (I < dim)      ! A(I:J) contains the range of repeated values
              ...
              !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
              N = J - I + 1
              CALL REVERSE_ORDER_1D_INT32( N, A(I:J) )
              !
  ...
  MODULE PURE SUBROUTINE REVERSE_ORDER_2D_INT32(A, REVERSE_EACH_ROW)
    ...
    IF(present(REVERSE_EACH_ROW)) THEN
      IF(REVERSE_EACH_ROW)  THEN
                            CALL REVERSE_ORDER_2D_EACH_ROW_INT32(dim1,dim2,A)
                            RETURN
      END IF
    END IF
    !
    CALL REVERSE_ORDER_2D_EACH_COL_INT32(dim1,dim2,A)
    !
  END SUBROUTINE
  !
  !--------------------------------------------------------------------------------------------------------------------------
  ...
  MODULE PURE SUBROUTINE REVERSE_ORDER_2D_INDEX_INT32(A, REV_INDEX, STABLE, REVERSE_ROW, P)
    ...
    IF( dim1 == 1 .OR. dim2 == 1 ) THEN  ! Array really is 1D, so use those optimized routines
        IF(dim1 > 1) THEN
                CALL REVERSE_ORDER_1D_NODIM_INT32( A(:,1), STABLE, P )
        ELSE
                CALL REVERSE_ORDER_1D_NODIM_INT32( A(1,:), STABLE, P )
        END IF
        RETURN
    ...
    !
    IF( revCOL ) THEN
        CALL REVERSE_ORDER_2D_COL_INT32(dim1, dim2, A, revIDX, STABLE, P)
    ELSE
        CALL REVERSE_ORDER_2D_ROW_INT32(dim1, dim2, A, revIDX, STABLE, P)
    END IF
    !
  END SUBROUTINE
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_COL_INT32(dim1, dim2, A, COL, STABLE, PRM)
    INTEGER,                                            intent(in   ) :: dim1, dim2, COL
    INTEGER(INT32), dimension(dim1,dim2),               intent(inout) :: A
    LOGICAL,                                  optional, intent(in   ) :: STABLE
    ...
    INTEGER:: k
    !
    CALL REVERSE_ORDER_1D_DIM_INT32(dim1, A(:,COL), STABLE, P)
    !
    ! Sort the other columns by permutation vector
    DO k=1, COL-1
                CALL PERM_SORT(dim1, A(:,k), P)
  ...
  !--------------------------------------------------------------------------------------------------------------------------
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_ROW_INT32(dim1, dim2, A, ROW, STABLE, PRM)
    INTEGER,                                            intent(in   ) :: dim1, dim2, ROW
    INTEGER(INT32), dimension(dim1,dim2),               intent(inout) :: A
    LOGICAL,                                  optional, intent(in   ) :: STABLE
    INTEGER,        dimension(:), contiguous, optional, intent(inout) :: PRM
    ...
    call ROW_COPY(dim1, dim2, row, A, TMP)
    !
    call REVERSE_ORDER_1D_DIM_INT32(dim2, TMP(1:dim2), STABLE, P)
    !
    IF(present(PRM))THEN
  ...
  !--------------------------------------------------------------------------------------------------------------------------
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_EACH_COL_INT32 (dim1,dim2,A)
  ...
  !--------------------------------------------------------------------------------------------------------------------------
  !
  PURE SUBROUTINE REVERSE_ORDER_2D_EACH_ROW_INT32(dim1, dim2, A)
    INTEGER,                              intent(in   ) :: dim1, dim2
    INTEGER(INT32), dimension(dim1,dim2), intent(inout) :: A
    INTEGER:: I, J, HALF_DIM
    !
    ...
    !
    DO I=1, HALF_DIM
            CALL VECTOR_SWAP_INT32( dim1, A(:,I), A(:,J) )
            J = J - 1
    END DO
    !
  END SUBROUTINE
  ...
  PURE SUBROUTINE RotrateBlock(dim, A, L, M, R)
    ...
    IF( M == R ) THEN
                 DO I=R-1, L, -1
                         CALL SWAP_INT32(dim, A, I, I+1)     ! CALL SWAP_INT32(A(I), A(I+1))
                 END DO
                 RETURN
    END IF
    !
  ...
  PURE SUBROUTINE BlockSwap(dim, A, fi, si, siz)
    ...
    INTEGER:: I
    DO I=0, siz-1
            CALL SWAP_INT32(dim, A, fi+I, si+I)     !CALL SWAP_INT32(A(fi+I), A(si+I))
    END DO
  END SUBROUTINE
  !
  !##########################################################################################################################
  ...
  !
  !PURE SUBROUTINE SWAP_INT32(A, B)          ! Can cause compiler errors due to vectorization when passing array parts, such as CALL SWAP_INT32( A(fi+I), A(si+I) )
  ...
  END SUBROUTINE
  !
  PURE SUBROUTINE VECTOR_SWAP_INT32(dim, A, B)
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(inout) :: A, B
    INTEGER(INT32):: TMP
    INTEGER:: I
    !
    DO I=1, dim
  ...
  PURE SUBROUTINE ROW_COPY_TO_VEC(dim1, dim2, row, A, B)
    INTEGER,                               intent(in ) :: dim1, dim2, row
    INTEGER(INT32), dimension(dim1, dim2), intent(in ) :: A
    INTEGER(INT32), dimension(dim2),       intent(out) :: B
    INTEGER:: i
    !
  ...
  PURE SUBROUTINE ROW_COPY_FROM_VEC(dim1, dim2, row, A, B)
    INTEGER,                               intent(in ) :: dim1, dim2, row
    INTEGER(INT32), dimension(dim2),       intent(in ) :: A
    INTEGER(INT32), dimension(dim1, dim2), intent(out) :: B
    INTEGER:: i
    !
    DO CONCURRENT(i=1:dim2)
                  B(row,i) = A(i)
  ...
  !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  !
  ! SORT_B_by_A_and_P_xyz_INT32 Routines
  !
    ...
    DIM_SEARCH: DO WHILE (I < dim)  ! A(I:J, col) contains the range of repeated values
       ...
       IF(HAS_P) THEN
          ...
          ELSE                                        ! Repeated values from I to J
              ...
              !
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1P_INT32   (N, B(I:J), P(I:J))
              ELSE
                  CALL SymMerge_1P_INT32(N, B(I:J), P(I:J))
              END IF
              !
              IF(dscnd) B(I:J) = -B(I:J)
          END IF
       ELSE!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          ELSE                                        ! Repeated values from I to J
              ...
              !
              IF ( N < ISORT_SMALL_ARRAY_SIZE) THEN
                  CALL ISORT_1D_INT32   (N, B(I:J))
              ELSE
                  CALL SymMerge_1D_INT32(N, B(I:J))
              END IF
              !
              IF(dscnd) B(I:J) = -B(I:J)
          END IF
  ...
  !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  !
  ! SORT_B_by_REP_and_P_INT32 Routines
  !
  ...
  END SUBROUTINE
  !
  PURE SUBROUTINE UPDATE_REP_INT32(dim, A, REP, repeat_found) !Given a sorted A, update REP to include its repeated values that match with non-zero REP
    INTEGER,                        intent(in   ) :: dim
    INTEGER(INT32), dimension(dim), intent(in   ) :: A
    INTEGER,        dimension(dim), intent(inout) :: REP
  ...
  MODULE PURE SUBROUTINE SORT_B_by_A_and_P_REP_INT32(dim, REP, B, repeat_sort, P, descend)
    ...
    END DO DIM_SEARCH
    !
    IF(repeat_found) CALL UPDATE_REP_INT32(dim, B, REP, repeat_found)
    !
  ...
  !!
  !PURE FUNCTION REPEATED_RANGE_INT32(dim, A, I) RESULT(J)  ! aka JSEARCH
...
!
!
!!!  ! Routine determines the TimSort minrun length, which is one less than
!!!  ! If n < 64, return n.
!!!  ! Else if n is an exact power of 2, return 32.
!!!  ! Else return an int k, 32 <= k <= 64, such that n/k is close to, but
!!!  ! strictly less than, an exact power of 2.
!!!  PURE FUNCTION TIMSORT_MINRUN(dim)  result(N)  ! dim must be >0
!!!    INTEGER, intent(in):: dim
!!!    INTEGER:: N
!!!    INTEGER:: r
!!!    !
...
!!!  END FUNCTION
  ...
  !
  !!!PURE FUNCTION USE_ISORT(dim, A) RESULT(ANS)
  !!!  INTEGER,                        intent( in):: dim
  !!!  INTEGER(INT32), dimension(dim), intent( in):: A
  !!!  LOGICAL:: ANS
  !!!  INTEGER:: I, CNT, LIM
  !!!  !
  !!!  ANS = dim <= ISORT_SMALL_ARRAY_SIZE                          ! Small Array
  !!!  !
  !!!  IF(.NOT. ANS ) THEN
  !!!                LIM = INT( PARTIAL_SORTED_LIM * REAL(dim) )
  !!!                IF(LIM < 5) LIM = 5
  ...
  !!!                             IF( A(I) > A(I+1) )  CNT = CNT + 1  ! Is zero if perfectly sorted, and dim/2 if completely not-sorted
  !!!                END DO
  !!!                ANS = CNT <= LIM    ! Well sorted array has increasing values less than LIM % of the time.
  !!!  END IF
  !!!END FUNCTION


!!!  !
!!!  PURE SUBROUTINE SIGN_FLIP_2D(dim1, dim2, A, SORT_INDEX, SORT_BY_COL)                 ! Assumes SORT_INDEX /= 0
!!!    INTEGER,                             intent(in   ):: dim1, dim2, SORT_INDEX
!!!    INTEGER(INT32), dimension(dim1,dim2),intent(inout):: A
!!!    LOGICAL,                             intent(in   ):: SORT_BY_COL
!!!    INTEGER:: I, P
!!!    IF( SORT_INDEX > 0 ) THEN
!!!                         P =    SORT_INDEX
!!!    ELSE
...
!!! STABLE QSORT Replaced by SYMMERGE

!!!  !
!!!  !--------------------------------------------------------------------------------------------------------------------------
!!!  ! Stable QSORT Routine - Requires IDX Array to keep track of position
!!!  !
!!!  PURE SUBROUTINE QSORT_STABLE_IDX_1D_INT32(dim, A, P)
!!!    INTEGER,                        intent(in   ):: dim
!!!    INTEGER(INT32), dimension(dim), intent(inout):: A
!!!    INTEGER,        dimension(dim), intent(inout):: P
!!!    INTEGER:: N, I, J, L, R, POS   ! Note I -> L
!!!    !
!!!    CALL QSORT_1P_INT32( dim, A, P, 1, dim, MAX_RECUR(dim) )
!!!    !
!!!    ! Check and sort the Index Vector P
!!!    !
!!!    N = 1
!!!    DO WHILE ( N < dim )
!!!         IF( A(N) /= A(N+1) ) THEN   ! Next Value not the same, continue
!!!                         N = N + 1
!!!         ELSE                        ! Search for next different value
!!!            L = N                    ! Store first same value
!!!            N = N + 1
!!!            DO WHILE ( N < dim )
!!!                       IF( A(N) /= A(N+1) ) EXIT
!!!                       N = N + 1
...
!!!            R = N                    ! Last same value
!!!            !
!!!            IF( R - L == 1) THEN                ! Only 2 values the same -> flip if necessary
!!!                            IF(P(L) > P(R)) THEN
!!!                                             POS = P(L);  P(L) = P(R);  P(R) = POS
!!!                            END IF
!!!            ELSE IF(R > L) THEN
!!!                            DO I=L+1, R       ! Insertion Sort Indexes -> Assume L-R is not large
!!!                                POS = P(I)
!!!                                J   = I - 1
!!!                                DO WHILE ( J >= L )
!!!                                           IF( POS >= P(J) ) EXIT
...
! Index Sort Routines Superceded by PERMUTAITON SORT Method

!!!  INTEGER, PARAMETER:: INDEX_SORT_COPY_LIMIT  = 64   ! Index SORT routines will make a copy of IDX if dim <= LIM, otherwise a special recursive routine is used to save space
!!!
!!!  MODULE PURE SUBROUTINE INDEX_SORT_1D_INT32(A, IDX, SAVE_IDX)
!!!    INTEGER(INT32), dimension(:), contiguous, intent(inout):: A
!!!    INTEGER,        dimension(:), contiguous, intent(inout):: IDX
!!!    LOGICAL,                        optional, intent(in   ):: SAVE_IDX
!!!    INTEGER:: dim
!!!    !
!!!    dim = SIZE(A)
!!!    !
!!!    IF(SIZE(IDX) < dim) THEN    !Check for Programming Error
!!!                        dim = 0
!!!    ELSEIF(IDX(1) < 1 ) THEN
!!!                        dim = 0
!!!    END IF
...
!!!    END IF
!!!    !
!!!    IF(present(SAVE_IDX)) THEN
!!!      IF(.NOT. SAVE_IDX)  THEN
!!!                          CALL INDEX_SORT_1D_DESTROY_IDX(dim, A, IDX)  ! Sort quickly, but sacrafice IDX
!!!                          RETURN
!!!      END IF
!!!    END IF
!!!    !
!!!    IF( dim <= INDEX_SORT_COPY_LIMIT) THEN
!!!        CALL INDEX_SORT_1D_COPY_IDX(dim, A, IDX)
!!!    ELSE
!!!        CALL INDEX_SORT_1D_RECUR_IDX_INT32(dim, A, IDX)
!!!    END IF
!!!    !
!!!  END SUBROUTINE
!!!  !
!!!  !--------------------------------------------------------------------------------------------------------------------------
!!!  !
!!!  PURE SUBROUTINE INDEX_SORT_1D_COPY_IDX(dim, A, IDX)
!!!    INTEGER,                        intent(in   ):: dim
!!!    INTEGER(INT32), dimension(dim), intent(inout):: A
!!!    INTEGER,        dimension(dim), intent(inout):: IDX
!!!    INTEGER,        dimension(dim):: POS
!!!    !
!!!    POS = IDX
!!!    CALL INDEX_SORT_1D_DESTROY_IDX(dim, A, POS)
!!!    !
!!!  END SUBROUTINE
!!!  !
!!!  PURE SUBROUTINE INDEX_SORT_1D_DESTROY_IDX(dim, A, IDX)
!!!    INTEGER,                        intent(in   ):: dim
!!!    INTEGER(INT32), dimension(dim), intent(inout):: A
!!!    INTEGER,        dimension(dim), intent(inout):: IDX
!!!    !
...
!!!  !--------------------------------------------------------------------------------------------------------------------------
!!!  !
!!!  MODULE PURE SUBROUTINE INDEX_SORT_1D_RECUR_IDX_INT32(dim, A, IDX)
!!!    INTEGER,                        intent(in   ):: dim
!!!    INTEGER(INT32), dimension(dim), intent(inout):: A
!!!    INTEGER,        dimension(dim), intent(inout):: IDX
!!!    INTEGER:: P
!!!    !
!!!    DO CONCURRENT (P=1:dim, IDX(P) == P)
!!!                            IDX(P) = -1*IDX(P)
!!!    END DO
!!!    !
!!!    DO P=1, dim
!!!        IF( IDX(P) > 0 ) CALL INDEX_SORT_1D_REC(dim, A, IDX, P)
!!!    END DO
!!!    !
!!!    DO CONCURRENT (P=1:dim)
!!!                       IDX(P) = -1*IDX(P)
!!!    END DO
!!!    !
!!!  END SUBROUTINE
!!!  !
!!!  RECURSIVE PURE SUBROUTINE INDEX_SORT_1D_REC(dim, A, IDX, P)
!!!    INTEGER,                        intent(in   ):: dim, P
!!!    INTEGER(INT32), dimension(dim), intent(inout):: A
!!!    INTEGER,        dimension(dim), intent(inout):: IDX
!!!    INTEGER(INT32):: TMP
!!!    INTEGER:: POS
...
!!!    IDX(P) = -1 * POS
!!!    !
!!!    CALL INDEX_SORT_1D_REC(dim, A, IDX, POS)
!!!    !
!!!    A(P) = TMP
!!!    !
!!!  END SUBROUTINE
...
!!!  !##########################################################################################################################
!!!  !
!!!  MODULE PURE SUBROUTINE INDEX_SORT_2D_INT32(A, IDX, SORT_BY_ROW)
!!!    INTEGER(INT32), dimension(:,:), contiguous, intent(inout):: A
!!!    INTEGER,        dimension(:),   contiguous, intent(inout):: IDX
!!!    LOGICAL,                          optional, intent(in   ):: SORT_BY_ROW
!!!    INTEGER:: dim1, dim2
!!!    LOGICAL:: BY_COL
!!!    !
!!!    dim1 = SIZE(A,1)
...
!!!    !
!!!    BY_COL = .TRUE.
!!!    IF(present(SORT_BY_ROW)) BY_COL = .NOT. SORT_BY_ROW
!!!    !
!!!    IF(  (       BY_COL .AND. SIZE(IDX) < dim1 ) .OR. &            ! Programming Error
!!!         ( .NOT. BY_COL .AND. SIZE(IDX) < dim2 )       ) THEN
!!!                                                         IDX = -1
!!!                                                         A   = NEG
!!!                                                         RETURN
!!!    END IF
...
!!!       !
!!!       IF(BY_COL) THEN
!!!            CALL INDEX_SORT_2D_COL_INT32(dim1, dim2, A, IDX)
!!!       ELSE
!!!            CALL INDEX_SORT_2D_ROW_INT32(dim1, dim2, A, IDX)
!!!       END IF
!!!       !
!!!    ELSE IF(dim1 > 1) THEN                                       ! Array really is 1D, so use those optimized routines
!!!                      CALL INDEX_SORT_1D_RECUR_IDX_INT32(dim1, A(:,1), IDX)
!!!    ELSE!IF(dim2 > 1) THEN
!!!                      CALL INDEX_SORT_1D_RECUR_IDX_INT32(dim2, A(1,:), IDX)
!!!    END IF
!!!    !
!!!  END SUBROUTINE
!!!  !
!!!  !--------------------------------------------------------------------------------------------------------------------------
!!!  !
!!!  PURE SUBROUTINE INDEX_SORT_2D_COL_INT32(dim1, dim2, A, IDX)
!!!    INTEGER,                              intent(in   ):: dim1, dim2
!!!    INTEGER(INT32), dimension(dim1,dim2), intent(inout):: A
!!!    INTEGER,        dimension(dim1),      intent(inout):: IDX
!!!    INTEGER,        dimension(dim1):: P
!!!    INTEGER:: J
!!!    !
!!!    DO J=1, dim2
!!!            P = IDX
!!!            CALL INDEX_SORT_1D_DESTROY_IDX(dim1, A(:,J), P)
!!!    END DO
!!!    !
!!!  END SUBROUTINE
!!!  !
!!!  !--------------------------------------------------------------------------------------------------------------------------
!!!  !
!!!  PURE SUBROUTINE INDEX_SORT_2D_ROW_INT32(dim1, dim2, A, IDX_ORIG)
!!!    INTEGER,                              intent(in   ):: dim1, dim2
!!!    INTEGER(INT32), dimension(dim1,dim2), intent(inout):: A
!!!    INTEGER,        dimension(dim2),      intent(in   ):: IDX_ORIG
!!!    INTEGER,        dimension(dim2):: IDX
!!!    INTEGER:: I, P, POS
!!!    !
!!!    !
!!!    IDX = IDX_ORIG
...
!!!            !
!!!            POS = IDX(P)
!!!            CALL VECTOR_SWAP_INT32(dim1, A(:,P), A(:,POS))
!!!            !
!!!            DO I=P+1, dim2
!!!                    IF(IDX(I) == P) THEN
!!!                              IDX(I) = POS
