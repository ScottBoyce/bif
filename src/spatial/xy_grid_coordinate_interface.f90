...
MODULE XY_GRID_COORDINATE_INTERFACE
  ...
  !
  DOUBLE PRECISION,PARAMETER::EPS=1.D-5                             !TOLERANCE FOR APPROXIMATING SINE/COSINE TO EXACT VALUE WHEN NEARBY 0, 90, 180, and 270 degrees
  ...
  DOUBLE PRECISION,PARAMETER::MINBOT = 1.D-30                       !USED TO DETERMINE SLOPE OF GRID. IF RUN IS LESS THAN THIS THE VALUE IS USED INSTEAD. PREVENTS DIV/0 ERROR
  DOUBLE PRECISION,PARAMETER::nMINBOT=-1.D-30
  !
  ...
  SUBROUTINE BUILD_XYCOORD_FROM_INITIAL_POINT(XY,Xin,Yin,ROT,LLCOODRINATE,CORNERCOORD,DELR,DELC)
    ...
    !YPCORD=>XY%YCORN
    !
    IF(ABS(ROT)<EPS .OR. ABS(360.D0-ROT)<EPS)THEN                      !TO PREVENT NUMERICAL ERRORS SET OBVIOUS SINE/COSINE VALUES
      SINROT  =0.D0
      COSROT  =1.D0
      SIN90ROT=-1.D0
      COS90ROT=0.D0
    ELSEIF(ABS(90.D0 -ROT)<EPS)THEN
      SINROT  =1.D0
      COSROT  =0.D0
      SIN90ROT=0.D0
      COS90ROT=1.D0
    ELSEIF(ABS(180.D0-ROT)<EPS)THEN
      SINROT  =0.D0
      COSROT  =-1.D0
      SIN90ROT=1.D0
      COS90ROT=0.D0
    ELSEIF(ABS(270.D0-ROT)<EPS)THEN
      SINROT  =-1.D0
      COSROT  =0.D0
      SIN90ROT=0.D0
      COS90ROT=-1.D0
    ELSE
      SINROT  =SIN(ROT*RAD)
      COSROT  =COS(ROT*RAD)
      SIN90ROT=SIN((ROT-90.D0)*RAD)
      COS90ROT=COS((ROT-90.D0)*RAD)
    END IF
    !
    XY%SINROT   = SINROT  
    XY%COSROT   = COSROT  
