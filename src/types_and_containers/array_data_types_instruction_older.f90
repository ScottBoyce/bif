...
MODULE ARRAY_DATA_TYPES!, ONLY: INTEGER_VECTOR, DOUBLE_VECTOR, INTEGER_MATRIX, DOUBLE_MATRIX, LOGICAL_VECTOR, CHARACTER_TYPE, CHARACTER_TYPE_ARRAY, CHARACTER_ARRAY, COMPRESSED_VALUE_STORAGE
  ...
  USE, INTRINSIC:: ISO_FORTRAN_ENV, ONLY: INT32, INT64, DBL => REAL64
  !USE CONSTANTS
  IMPLICIT NONE
  !
  PUBLIC:: JOIN_CHAR
  ...
  TYPE CHARACTER_TYPE
      ...
      PROCEDURE, PASS(CH) :: DESTROY   => DEALLOCATE_CHARACTER_TYPE          ! CALL CH%DESTROY(CH_OUT)
      !                   
  ...
  TYPE CHARACTER_TYPE_ARRAY
      ...
      PROCEDURE, PASS(CH) :: UNIQUE  => UNIQUE_CHARACTER_TYPE_ARRAY     ! %UNIQUE(STR)
      PROCEDURE, PASS(CH) :: GET     => GET_CHARACTER_TYPE_ARRAY        ! %GET(POS)
      GENERIC ::             POP     => REMOVE_LAST_POSITION,     &
                                        REMOVE_SPECIFIED_POSITION       ! %POP([LINE])
  ...
  ELEMENTAL PURE FUNCTION CONCAT_STR_CHARACTER_TYPE(STR, CH) RESULT(STR_OUT)
     CLASS(CHARACTER_TYPE), INTENT(IN):: CH
     CHARACTER(*),          INTENT(IN):: STR
     CHARACTER( MAX(LEN(STR)+CH%N, 1) ):: STR_OUT
     !
     IF(CH%N > Z) THEN
                  STR_OUT = STR // CH%STR
     ELSE
  ...
  END FUNCTION
  !
  ELEMENTAL PURE FUNCTION CONCAT_CHARACTER_TYPE(CH1, CH2) RESULT(CH_OUT)
     CLASS(CHARACTER_TYPE), INTENT(IN):: CH1, CH2
     CHARACTER(MAX(CH1%N + CH2%N, 1)):: CH_OUT
     !
     IF    (CH1%N > Z .AND. CH2%N > Z) THEN
                                       CH_OUT = CH1%STR // CH2%STR
     ELSEIF(CH1%N > Z                ) THEN
                                       CH_OUT = CH1%STR
     ELSEIF(                CH2%N > Z) THEN
                                       CH_OUT = CH2%STR
...
!  END SUBROUTINE
  ...
  PURE SUBROUTINE ADD_DBL_CHARACTER_TYPE_ARRAY(CH, VAL, PAD)  
     ...
     END IF
     !
     CH%VEC(CH%N)%N   = LEN(CH%VEC(CH%N)%STR)
     !
  END SUBROUTINE
  !
  ...
  PURE FUNCTION NEW_DIM_SIZE(DIM) RESULT(DIM_NEW)
     ...
     ELSEIF(DIM < 4096) THEN
                        DIM_NEW =  512*(ONE + DIM/512)  + 512
     ELSE
                        DIM_NEW = 1024*(ONE + DIM/1024) + 1024
     END IF
     !
  END FUNCTION
  !    
  ...
  END FUNCTION
  !    
  PURE FUNCTION UNIQUE_CHARACTER_TYPE_ARRAY(CH, STR) RESULT(UNIQUE)
     CLASS(CHARACTER_TYPE_ARRAY), INTENT(IN):: CH
     CHARACTER(*),                INTENT(IN):: STR
     LOGICAL:: UNIQUE
     INTEGER:: CNT, I
