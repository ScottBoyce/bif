...
MODULE TRANSIENT_FILE_READER_INSTRUCTION
  ...
  SUBROUTINE LOAD_NEXT_TFR_BLOCK(TFR, BL, LLOC, IU, SKIP_END_KEY)  !ONLY CALLED FROM LOAD_NEXT_FILE_SCALAR
    ...
    CALL LOAD_NEXT_FILE_BLOCK(BL, LLOC, IU, TFR%OLD_IU, TFR%LN, TFR%IOUT, TFR%IU, TFR%NULL_FILE, TFR%REPEAT, TFR%ENTIRE_LINE, TFR%TRUNCATED, TFR%BEGIN_END_OPTIONAL, TFR%BLOCK_END_KEY, TFR%NOID, TFR%EX1_WORD, TFR%EX1_DIM, TFR%EX2_WORD, TFR%EX2_DIM, TFR%EX3_WORD, TFR%EX3_DIM, TFR%NO_BINARY, SKIP_END_KEY)
    !
  END SUBROUTINE
  !
  SUBROUTINE LOAD_NEXT_FILE_BLOCK(BL, LLOC, IU, OLD_IU, LINE, IOUT, IN, SKIP, IS_REPEAT, FOUND_BEGIN, FOUND_END, BEGIN_END_OPTIONAL, BLOCK_END_KEY, NOID, EX1_WORD, EX1_DIM, EX2_WORD, EX2_DIM, EX3_WORD, EX3_DIM, NO_BINARY, SKIP_END_KEY)
    ...
    ELSEIF (IU == Z) THEN
        ...
        !
        IF(FL%IS_CONSTANT) CALL STOP_ERROR( LINE=LINE, OUTPUT=BL%IOUT, MSG= 'TRANSIENT FILE READER - BLOCK READER ERROR:  THE KEYWORD "CONSTANT" IS NOT ALLOWED.')
        !
