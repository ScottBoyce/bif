...
SUBROUTINE test_GENERIC_INPUT_FILE_INSTRUCTION(UT)
  ...
  INTEGER:: IU
  DOUBLE PRECISION:: DTMP
  CHARACTER(36):: FILE, UNIT
  FILE = './input/test_generic_input_file.txt'
  !
  CALL UT%NEXT_SECTION("GENERIC_INPUT_FILE_INSTRUCTION")
    !
    CALL UT%NEXT_TEST("BASIC OPEN DIRECTIVE")
        CALL FL%OPEN('OPEN/CLOSE ' //FILE) 
        CALL UT%ASSERT(LINE_TESTS(FL), MSG='OPEN/CLOSE  file')
        CALL FL%CLOSE()

        CALL FL%OPEN('DATAFILE  ' //FILE) 
        CALL UT%ASSERT(LINE_TESTS(FL), MSG='DATAFILE  file')
        CALL FL%REWIND()
        WRITE(UNIT, *) FL%IU                                ! File stays open with DATAUNIT
        UNIT = ADJUSTL(UNIT)
        CALL FL%CLOSE()

        CALL FL%OPEN('EXTERNAL  ' //UNIT) 
        CALL UT%ASSERT(LINE_TESTS(FL), MSG='EXTERNAL  file')
        CALL FL%REWIND()
        CALL FL%CLOSE()

        CALL FL%OPEN('DATAUNIT  ' //UNIT) 
        CALL UT%ASSERT(LINE_TESTS(FL), MSG='DATAUNIT  file')
        CALL FL%CLOSE()
        
        
        ! CALL UT%NEXT_TEST("INTERNAL")
  
  CONTAINS
  
  FUNCTION LINE_TESTS(FL) RESULT(ANS)
    TYPE(GENERIC_INPUT_FILE), INTENT(INOUT):: FL
    LOGICAL:: ANS
    INTEGER, DIMENSION(3):: IVEC
    CHARACTER(32):: LINE
    LOGICAL:: EOF, TRUE, FALSE
    TRUE = .TRUE.
    FALSE = .FALSE.
    
    ANS = TRUE
    
    CALL FL%READ(LINE, READ_COM=TRUE)
    IF (LINE /= '# Comment Header') ANS = FALSE
    
    CALL FL%READ(LINE, READ_COM=TRUE)
    IF (LINE /= 'Line 1') ANS = FALSE
    
    CALL FL%READ(LINE, READ_COM=TRUE)
    IF (LINE /= 'Line 2   # Side Comment') ANS = FALSE
    
    CALL FL%READ(LINE, READ_COM=TRUE)
    IF (LINE /= 'Line 3') ANS = FALSE
    
    CALL FL%READ(LINE, READ_COM=TRUE)
    IF (LINE /= '') ANS = FALSE
    
    CALL FL%READ(LINE, READ_COM=TRUE)
    IF (LINE /= '1 2 3    # Side Comment') ANS = FALSE
    
    CALL FL%READ(LINE, EOF=EOF, READ_COM=TRUE)
    IF (.not. EOF) ANS = FALSE
